...
var sup=document.querySelector('.maj-profil-a')
...
deconnexion.addEventListener("click", function(event) {
  ...
  // Si l'utilisateur clique sur "OK", effectuer la déconnexion
  if (confirmation) {
    // Rediriger vers la page de déconnexion
    window.location.href = deconnexion.getAttribute("href");
  }
})

const imageInput = document.getElementById('image-input');
const urlImageInput = document.querySelector('input[name="urlImage"]');

imageInput.addEventListener('change', (event) => {
  const file = event.target.files[0];
  const relativePath = `/image/${file.name}`;
  urlImageInput.value = relativePath;
});
